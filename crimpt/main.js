...
var inr = ()=>{
	...
	for(var i=M.unit.length-1;i>=0;i--){
		...
		if(u.byPlayer){ //pass Playerial Intelligence
			...
			// Set the aim to follow the player unit
			var playerUnit = M.unit[M.playerIndex];
			
			// Only update target if there are no aims or distance to target is large
			if (u.aims.length === 0 || 
			    (u.aims[0].name === 'go' && 
			     (Math.abs(u.aims[0].P[0] - playerUnit.P[0]) > 1 || 
			      Math.abs(u.aims[0].P[1] - playerUnit.P[1]) > 1))) {
				u.aims = [{
					name : 'go',
					P : [...playerUnit.P],
				}];
			...
			while(j<u.aims.length){
				...
				switch(A.name){
				 case 'follow':
					var targetUnit = M.unit[A.target];
					if (!targetUnit) {
						u.aims.shift();
						continue;
					}
